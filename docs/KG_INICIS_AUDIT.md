...
| # | 항목 | URL / 방법 | 계정 | 확인 포인트 |
|---|------|------------|------|-------------|
| 1 | 회원가입 고객 구분 | https://infront.kr/signup?audit=1 | (신규 가입 화면만) | 내국인 / 외국인·해외고객 드롭다운, URL 바, 사업자정보 |
| 2 | 마이페이지 고객 구분 | https://infront.kr/mypage?audit=1 | 아무 회원 | 「고객 구분 (결제)」 카드, URL 바, 사업자정보 |
| 3 | checkout — 내국인 | https://infront.kr/shop/checkout?product=INSPECTION_PACK_S&audit=1 | `audit-domestic@...` | 고객 유형=내국인, **해외 결제수단 미노출**, 원화 금액 안내 |
| 4 | checkout — 외국인 | 동일 URL | `audit-foreigner@...` | 고객 유형=외국인, **해외카드·글로벌 결제 노출** |
| 5 | KG이니시스 결제창 (BC/하나) | checkout(내국인, 신용카드) → 결제 진행 | `audit-domestic@...` | 카드사 목록 / BC 페이북 / BC 페이북 앱 / 하나 안심클릭 (4장) |
| 6 | 주문 목록 | https://infront.kr/shop/orders?audit=1 | 외국인 계정 | 주문·고객 유형 표시 |
| 7 | 배송·환불 정책 | `/shop/shipping-policy?audit=1`, `/shop/refund-policy?audit=1` | — | 정책 페이지, URL 바, 사업자정보 |
| 8 | 상품·안내 | https://infront.kr/shop?audit=1 | — | 해외카드 안내 문구, 사업자정보 |

> admin(관리자) 화면 캡처는 이번 심사 캡처 요건(URL/사업자정보/원화금액/BC·하나 결제창)에 **포함되지 않음** — 캡처 불필요.
...
| 3 | production 배포 (`/mypage` 고객 구분 카드 노출) | ✅ (2026-07-14) |
| 4 | BC/하나 카드사 결제창 캡처용 테스트 결제 1건 진행 (내국인 계정, 신용카드) | ☐ |
| 5 | 외국인 계정 이메일 인증 완료 상태 확인 | ✅ (스크립트로 보장) |

### C. production 배포 확인

```text
https://infront.kr/mypage  → 로그인 후 「고객 구분 (결제)」 카드 보임
https://infront.kr/shop/checkout?product=INSPECTION_PACK_S → 「회원 정보 기준」 고객 유형 표시
```
...
- checkout (로그인 후): https://infront.kr/shop/checkout?product=INSPECTION_PACK_S
...
1. `/shop?audit=1` — URL 바 + 하단 사업자정보
2. `/signup?audit=1` — 고객 구분 + URL + 사업자정보
3. `/mypage?audit=1` — 고객 구분 카드
4. checkout 내국인/외국인 (`&audit=1`)
5. checkout **원화 금액** + KG 결제창 **동일 원화 금액** 나란히
6. **BC카드** 카드사 목록 / 페이북 / 페이북 앱 (3장)
7. **하나카드** 안심클릭 (1장)
8. 정책 페이지 (`/shop/shipping-policy`, `/shop/refund-policy`)

상세: `docs/audit-captures/KG_CAPTURE_GUIDE.md`
