 ...
 * 스토리지 페이지 — 서버 컴포넌트 래퍼
 *
 * 서버에서 데이터를 미리 조회해 StorageClient에 initialData로 전달.
 * 클라이언트는 스피너 없이 즉시 렌더링되며, 이후 탭 전환 시 조용히 새로고침.
 */
import { cookies } from "next/headers";
import { redirect } from "next/navigation";
import { createServerClient } from "@supabase/ssr";
import { getCachedStorageTypes } from "@/lib/storage/cached-types";
import StorageClient, { type DashboardData } from "./StorageClient";
...
export default async function StoragePage() {
  ...
  try {
    ...
    if (storagesResult.error || parcelsResult.error) {
      // 에러 시 클라이언트 사이드 fetch로 폴백
      return <StorageClient />;
    }

    // parcels → ProductItem[] 변환 (dashboard route와 동일)
    type PreInvoiceItem = {
      name?: string;
      product_name?: string;
      quantity?: number;
    ...
    const initialData: DashboardData = {
      ...
      storages: (storagesResult.data ?? []) as any,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      items: items as any,
      locationSummary: {
    ...
    return <StorageClient initialData={initialData} />;
  } catch {
    // 예외 시 클라이언트 사이드 fetch로 폴백
    return <StorageClient />;
  }
}
