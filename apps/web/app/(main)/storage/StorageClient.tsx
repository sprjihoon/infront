...
function SummaryTile({
  ...
  return (
    <div className="flex flex-col items-center text-center gap-1.5">
      <div className={`w-14 h-14 rounded-xl ${bg} flex items-center justify-center`}>{icon}</div>
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-shrink-0 relative w-10 h-10">
                    <div className="absolute bottom-0 left-0" style={{ filter: "drop-shadow(2px 3px 2px rgba(0,0,0,0.25))" }}>
                      <BrickSVG color="#f59e0b" typeCode={active[0]?.storage_types?.code ?? "MINI"} size={32} />
                    </div>
                    <div className="absolute top-0 right-0" style={{ filter: "drop-shadow(2px 3px 2px rgba(0,0,0,0.25))" }}>
                      <BrickSVG color="#ef4444" typeCode={active[1]?.storage_types?.code ?? "MINI"} size={28} />
                    </div>
                  </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-[480px] bg-white rounded-3xl shadow-2xl">
        ...
        <div className="px-4 py-5 space-y-4">
          ...
          {/* 블록 색상 */}
          <div>
            <p className="text-xs font-semibold text-gray-500 mb-2">블록 색상</p>
...
function MergeSlotSheet({
  ...
  const totalCapacity = storages.reduce((a, s) => a + (s.capacity_score ?? 0), 0);
  const mergedCapacity = targetStorage?.capacity_score ?? totalCapacity;
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-2">
                  ...
                  {suggestedType ? (
                    <div className="bg-white rounded-xl p-3 flex items-center gap-3 border border-amber-100">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-gray-800">{suggestedType.name} 으로 용량 변경 후 합치기</p>
                      ...
                      <button
                        onClick={() => onCapacityChange ? onCapacityChange(storages[0]) : onClose()}
