 ...
 *
 * 우체국 제공 PHP 샘플 코드를 TypeScript로 포팅 (modo 프로젝트 참조)
 */
...
const SS1 = new Uint32Array([
  ...
  0xd011c1d1,0x20200020,0xd417c7d3,0x00020202,0x20220222,0x04000404,0x68284860,0x70314171,
...
function GetB0(A: number) { return 0x000000ff & A; }
function GetB1(A: number) { return 0x000000ff & (A >>> 8); }
function GetB2(A: number) { return 0x000000ff & (A >>> 16); }
...
function SeedRound(
  L0: { value: number }, L1: { value: number },
  R0: { value: number }, R1: { value: number },
  K: number[]
) {
  let T0 = R0.value ^ K[0];
  let T1 = R1.value ^ K[1];
...
function SeedEncrypt(pbData: Uint8Array, pdwRoundKey: number[], outData: Uint8Array) {
  const L0 = { value: 0 }, L1 = { value: 0 }, R0 = { value: 0 }, R1 = { value: 0 };
  L0.value = (pbData[0]<<24)^(pbData[1]<<16)^(pbData[2]<<8)^pbData[3];
  L1.value = (pbData[4]<<24)^(pbData[5]<<16)^(pbData[6]<<8)^pbData[7];
  R0.value = (pbData[8]<<24)^(pbData[9]<<16)^(pbData[10]<<8)^pbData[11];
  R1.value = (pbData[12]<<24)^(pbData[13]<<16)^(pbData[14]<<8)^pbData[15];

  let n = 0;
  for (let i = 0; i < 16; i++) {
    const K = [pdwRoundKey[n++], pdwRoundKey[n++]];
    if (i % 2 === 0) SeedRound(L0, L1, R0, R1, K);
...
function EncRoundKeyUpdate0(
  K: number[], A: {value:number}, B: {value:number},
  C: {value:number}, D: {value:number}, Z: number
) {
  const T0 = A.value;
  A.value = ((A.value >>> 8) & 0x00ffffff) ^ (B.value << 24);
  B.value = ((B.value >>> 8) & 0x00ffffff) ^ (T0 << 24);
  const T00 = ConvertInt(A.value + ConvertInt(C.value) - ConvertInt(KC[Z]));
  const T11 = ConvertInt(B.value + ConvertInt(KC[Z]) - ConvertInt(D.value));
...
function EncRoundKeyUpdate1(
  ...
  const T0 = C.value;
  C.value = (C.value << 8) ^ ((D.value >>> 24) & 0xff);
  D.value = (D.value << 8) ^ ((T0 >>> 24) & 0xff);
  const T00 = ConvertInt(A.value + ConvertInt(C.value) - ConvertInt(KC[Z]));
  const T11 = ConvertInt(B.value + ConvertInt(KC[Z]) - ConvertInt(D.value));
  K[0] = SS0[GetB0(T00)]^SS1[GetB1(T00)]^SS2[GetB2(T00)]^SS3[GetB3(T00)];
  K[1] = SS0[GetB0(T11)]^SS1[GetB1(T11)]^SS2[GetB2(T11)]^SS3[GetB3(T11)];
...
function SeedRoundKey(pdwRoundKey: number[], pbUserKey: Uint8Array): number[] {
  ...
  const T1 = ConvertInt(B.value - D.value + KC[0]);
  ...
  while (n < 44) pdwRoundKey[n++] = 0;
  return pdwRoundKey;
}

function padding(data: Uint8Array): Uint8Array[][] {
  ...
  for (let i = 0; i < blocks; i++) {
    ...
    for (let j = 0; j < 16; j++) {
      const idx = i * 16 + j;
      block[j] = idx < data.length ? data[idx] : 0x00;
    }
    result.push([block]);
  }
  return result;
...
function byteArrayToHex(ba: Uint8Array): string {
  ...
  for (let i = 0; i < ba.length; i++) {
    s += ('0' + (0xff & ba[i]).toString(16)).slice(-2);
  }
  return s;
}
...
export function seed128Encrypt(plainText: string, key: string): string {
  ...
  const pbUserKey = encoder.encode(key);
  const key16 = new Uint8Array(16);
  for (let i = 0; i < 16; i++) key16[i] = i < pbUserKey.length ? pbUserKey[i] : 0;

  const pdwRoundKey = SeedRoundKey(new Array(44).fill(0), key16);
  const blocks = padding(encoder.encode(plainText));
  const out = new Uint8Array(blocks.length * 16);
  const tmp = new Uint8Array(16);

  for (let i = 0; i < blocks.length; i++) {
    SeedEncrypt(blocks[i][0], pdwRoundKey, tmp);
    out.set(tmp, i * 16);
  }
...
export function buildEpostParams(
  ...
  const required = new Set(['custNo','apprNo','recNm','recZip','recAddr1','recAddr2','recTel','orderNo']);
  const pairs: string[] = [];
