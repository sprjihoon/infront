...
// ── 요금 테이블 (창구접수 · 등기소포) ──────────────────────────────
// 크기: 가로+세로+높이 합계(cm) / 도서산간(제주 포함) +2,500원
const TIERS = [
  { id: 1, maxSizeCm: 80,  maxWeightG: 3000,  label: "80cm이하 · 3kg이하", regular: 4000  },
  { id: 2, maxSizeCm: 100, maxWeightG: 5000,  label: "~100cm · 5kg이하",   regular: 4500  },
  { id: 3, maxSizeCm: 100, maxWeightG: 7000,  label: "~100cm · 7kg이하",   regular: 5000  },
  { id: 4, maxSizeCm: 120, maxWeightG: 10000, label: "~120cm · 10kg이하",  regular: 6000  },
  { id: 5, maxSizeCm: 120, maxWeightG: 15000, label: "~120cm · 15kg이하",  regular: 7000  },
  { id: 6, maxSizeCm: 120, maxWeightG: 20000, label: "~120cm · 20kg이하",  regular: 8000  },
  { id: 7, maxSizeCm: 120, maxWeightG: 25000, label: "~120cm · 25kg이하",  regular: 11000 },
  { id: 8, maxSizeCm: 160, maxWeightG: 30000, label: "~160cm · 30kg이하",  regular: 13000 },
] as const;

const REMOTE_SURCHARGE = 2500;
...
export default function SidebarDomesticCalculator() {
  ...
  const [result,    setResult]    = useState<{ tier: typeof TIERS[number]; base: number; surcharge: number } | null>(null);
  ...
  function calc() {
    ...
    const tier = TIERS[Math.max(wTier, sTier) - 1];
    setResult({ tier, base: tier.regular, surcharge: remote ? REMOTE_SURCHARGE : 0 });
  }
    ...
    <div className="w-full bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden select-none">
      ...
      <div className="p-4 space-y-3">
        ...
        <details className="group">
          ...
          {volSizeCm !== null && (
            <p className="text-[10px] text-emerald-600 mt-1.5">합계 {volSizeCm.toFixed(0)}cm</p>
          )}
        </details>
        ...
        <label className="flex items-center gap-2 cursor-pointer select-none">
          <input
            type="checkbox"
            checked={remote}
            onChange={e => { setRemote(e.target.checked); reset(); }}
            className="w-4 h-4 rounded accent-amber-500"
          />
          <span className="text-xs text-gray-700 font-medium">도서산간 · 제주</span>
          <span className="text-[10px] text-amber-600 bg-amber-50 border border-amber-100 rounded px-1.5 py-0.5">
            +{REMOTE_SURCHARGE.toLocaleString()}원
          </span>
        </label>
                ...
                <div>
                  ...
                  {result.surcharge > 0 && (
                    <p className="text-[10px] text-amber-600">
                      {result.base.toLocaleString()} + {result.surcharge.toLocaleString()}원
                    </p>
                  )}
                </div>
              </div>
              <span className="text-lg font-bold text-emerald-900">
                {(result.base + result.surcharge).toLocaleString()}원
              </span>
            </div>
          ...
          <div className="mt-2 space-y-1">
            {TIERS.map(t => (
              <div key={t.id} className="flex items-center justify-between text-[10px] px-2 py-1 rounded-lg bg-gray-50 border border-gray-100">
                ...
                <span className="text-gray-700 font-medium shrink-0">{t.regular.toLocaleString()}원</span>
              </div>
            ))}
            <p className="text-[9px] text-amber-600 px-1 pt-0.5">※ 도서산간·제주 +2,500원</p>
          </div>
        </details>
      </div>
    </div>
